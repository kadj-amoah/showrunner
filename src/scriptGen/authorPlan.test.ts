...
describe('authorPlan', () => {
  it('authorPlan returns a grounded plan + the inventory it used', async () => {
    const res = await authorPlan(
      ...
      { scrape: async () => FAKE_INVENTORY, generate: async () => GOOD_MANIFEST as any },
    );
  ...
  it('authorPlan passes instructions through as guidance and the inventory to generate', async () => {
    ...
    await authorPlan(
      ...
      { scrape: async () => FAKE_INVENTORY, generate: async (opts: any) => { seenGuidance = opts.guidance; seenInv = opts.selectorInventory; return GOOD_MANIFEST as any; } },
    );
  ...
  it('authorPlan synthesizes a minimal valid product model when none is given', async () => {
    ...
    await authorPlan(
      ...
      { scrape: async () => FAKE_INVENTORY, generate: async (opts: any) => { seenPM = opts.productModel; return GOOD_MANIFEST as any; } },
    );
  ...
  it('authorPlan flags non-grounded when the inventory is empty (still returns, warnings set)', async () => {
    const res = await authorPlan(
      ...
      { scrape: async () => [], generate: async () => GOOD_MANIFEST as any },
    );
